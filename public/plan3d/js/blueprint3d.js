    ...
    (function (Three) {
        Three.Controller = function (three, model, camera, element, controls, hud) {
            ...
            function init() {
                ...
                element.dblclick(doubleClickEvent);
            ...
            function mouseMoveEvent(event) {
                if (scope.enabled) {
                    ...
                    mouseMoved = true;
                    // Get mouse coordinates relative to the element using jQuery offset (consistent with floorplanner)
            ...
            function doubleClickEvent(event) {
                if (scope.enabled) {
                    event.preventDefault();
                    // Update mouse position for double-click using jQuery offset (consistent with floorplanner)
                    var offset = element.offset();
                    mouse.x = event.clientX - offset.left;
                    mouse.y = event.clientY - offset.top;
                    // Always check walls and floors on double-click, regardless of state
                    checkWallsAndFloors();
                }
            }
            ...
            function normalizeVector2(vec2) {
                // Map from element-relative coordinates to normalized device coordinates
                // vec2 should already be relative to the element
                var retVec = new THREE.Vector2();
                var width = element.width();
                var height = element.height();
                // Convert from element-relative coordinates to normalized device coordinates [-1, 1]
                retVec.x = (vec2.x / width) * 2 - 1;
                retVec.y = -(vec2.y / height) * 2 + 1;
                return retVec;
            }
            //
