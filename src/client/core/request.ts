...
export const sendRequest = async <T>(
	...
	try {
		...
		// Check if user is already on auth pages to prevent unnecessary logout
		...
		// Auto-logout on authentication errors (401 Unauthorized, 403 Forbidden)
		if (!isOnAuthPage && (axiosError.response?.status === 401 || axiosError.response?.status === 403)) {
			const status = axiosError.response.status;
			const url = axiosError.config?.url || 'unknown';
			
			console.warn(`${status} ${status === 401 ? 'Unauthorized' : 'Forbidden'} detected, auto-logging out user`);
		...
		// Auto-logout on connection errors (API server down/unreachable)
		if (!isOnAuthPage && (axiosError.code === 'ERR_NETWORK' || 
			axiosError.message.includes('ERR_CONNECTION_REFUSED') ||
			axiosError.message.includes('net::ERR_CONNECTION_REFUSED'))) {
			
			console.warn('Connection refused - API server appears to be down, auto-logging out user');
			console.warn(`Failed request: ${axiosError.config?.url || 'unknown'}`);
